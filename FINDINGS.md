...
| NVIDIA Nemotron-3.5-Lightning-30B-A3B | MoE | ~33B / ~3B | UD-Q6_K_XL, Q8_K_XL | ctx 1M; completion-only |
...
- Nemotron is the fastest generator but has the slowest prompt-eval; Qwen3.6 has the best prompt-eval.
...
- **Nemotron fails Python at both quants** with an identical fatal bug:
  `from typing import list, dict, tuple` (lowercase names aren't exported → `ImportError`, 0/11).
  Because it's identical at Q6 and Q8, it's a **model trait, not a quant artifact**. Its Java is
  strong, though — Q6 Java (24/25) is its best work.
- **Q6 → Q8 did not help, and for Nemotron made things worse** (Java 24→21, Python 19→17). No model
  improved from the extra precision; quantization is not the lever for code quality here.
...
4. **Model traits persist across quants.** Nemotron's Python `ImportError` and its length-limit
   overshoot both appear identically at Q6 and Q8 — these are behaviors of the model, not the
   quantization.

### Overall recommendation for this hardware (agentic coding)

**Qwen3.6-35B-A3B at Q6** is the best all-rounder: MoE-fast (~53 t/s), 100% test pass rate across
Python and Java, strong code quality (24/23), and a solid in-range essay (24/25). Step up to
**Q8** only for the top code-quality ceiling; it's ~3% slower with no reliability gain. Nemotron is
the fastest and codes well in Java but **cannot be trusted for Python** (import bug) or for
**following explicit length/format constraints** (essay overshoot) — a meaningful liability for
agentic use. The dense models (Qwen3.8, Muse-Glimmer) write well but are too slow to be practical
here.
